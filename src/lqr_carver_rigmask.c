...
lqr_carver_rigmask_init(LqrCarver *r)
{
    ...
    LQR_CATCH_MEM(r->rigidity_mask = LRQ_CALLOC(float, r->w0 * r->h0));
...
/* LQR_PUBLIC */
LqrRetVal
lqr_carver_rigmask_add_xy(LqrCarver *r, double rigidity, int x, int y)
{
    int xt, yt;
    ...
    r->rigidity_mask[yt * r->w0 + xt] += (float) rigidity;
...
/* LQR_PUBLIC */
LqrRetVal
lqr_carver_rigmask_add_area(LqrCarver *r, double *buffer, int width, int height, int x_off, int y_off)
{
    int x, y;
    int xt, yt;
    int wt, ht;
    ...
    for (y = 0; y < y2 - y1; y++) {
        for (x = 0; x < x2 - x1; x++) {
            ...
            r->rigidity_mask[(yt + y1) * r->w0 + (xt + x1)] = (float) buffer[(y - y0) * width + (x - x0)];
        }
...
/* LQR_PUBLIC */
LqrRetVal
lqr_carver_rigmask_add(LqrCarver *r, double *buffer)
{
...
/* LQR_PUBLIC */
LqrRetVal
lqr_carver_rigmask_add_rgb_area(LqrCarver *r, uint8_t *rgb, int channels, int width, int height, int x_off,
                                int y_off)
{
    ...
    for (y = 0; y < y2 - y1; y++) {
        for (x = 0; x < x2 - x1; x++) {
            ...
            rigmask = (double) sum / (255 * c_channels);
            if (has_alpha) {
                rigmask *= (double) rgb[((y - y0) * width + (x - x0) + 1) * channels - 1] / 255;
            }

            xt = r->transposed ? y : x;
            yt = r->transposed ? x : y;

            r->rigidity_mask[(yt + y1) * r->w0 + (xt + x1)] = (float) rigmask;
...
/* LQR_PUBLIC */
LqrRetVal
lqr_carver_rigmask_add_rgb(LqrCarver *r, uint8_t *rgb, int channels)
{
