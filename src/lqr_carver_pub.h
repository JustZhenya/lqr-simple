...
LQR_PUBLIC LqrCarver *lqr_carver_new(uint8_t *buffer, int width, int height, int channels);
LQR_PUBLIC LqrCarver *lqr_carver_new_ext(void *buffer, int width, int height, int channels,
                                         LqrColDepth colour_depth);
LQR_PUBLIC void lqr_carver_destroy(LqrCarver *r);

/* initialize */
LQR_PUBLIC LqrRetVal lqr_carver_init(LqrCarver *r, int delta_x, float rigidity);

/* set attributes */
LQR_PUBLIC LqrRetVal lqr_carver_set_image_type(LqrCarver *r, LqrImageType image_type);
LQR_PUBLIC LqrRetVal lqr_carver_set_alpha_channel(LqrCarver *r, int channel_index);
LQR_PUBLIC LqrRetVal lqr_carver_set_black_channel(LqrCarver *r, int channel_index);
...
LQR_PUBLIC void lqr_carver_set_side_switch_frequency(LqrCarver *r, uint32_t switch_frequency);
LQR_PUBLIC LqrRetVal lqr_carver_set_enl_step(LqrCarver *r, float enl_step);
LQR_PUBLIC void lqr_carver_set_use_cache(LqrCarver *r, bool use_cache);
...
/* image manipulations */
LQR_PUBLIC LqrRetVal lqr_carver_resize(LqrCarver *r, int w1, int h1); /* liquid resize */
...
LQR_PUBLIC bool lqr_carver_scan(LqrCarver *r, int *x, int *y, uint8_t **rgb);
LQR_PUBLIC bool lqr_carver_scan_ext(LqrCarver *r, int *x, int *y, void **rgb);
LQR_PUBLIC bool lqr_carver_scan_line(LqrCarver *r, int *n, uint8_t **rgb);
LQR_PUBLIC bool lqr_carver_scan_line_ext(LqrCarver *r, int *n, void **rgb);
LQR_PUBLIC bool lqr_carver_scan_by_row(LqrCarver *r);
LRQ_DEPRECATED
LQR_PUBLIC int lqr_carver_get_bpp(LqrCarver *r);
LQR_PUBLIC int lqr_carver_get_channels(LqrCarver *r);
LQR_PUBLIC int lqr_carver_get_width(LqrCarver *r);
LQR_PUBLIC int lqr_carver_get_height(LqrCarver *r);
LQR_PUBLIC int lqr_carver_get_ref_width(LqrCarver *r);
LQR_PUBLIC int lqr_carver_get_ref_height(LqrCarver *r);
LQR_PUBLIC int lqr_carver_get_orientation(LqrCarver *r);
LQR_PUBLIC LqrColDepth lqr_carver_get_col_depth(LqrCarver *r);
LQR_PUBLIC LqrImageType lqr_carver_get_image_type(LqrCarver *r);
LQR_PUBLIC float lqr_carver_get_enl_step(LqrCarver *r);
LQR_PUBLIC int lqr_carver_get_depth(LqrCarver *r);
